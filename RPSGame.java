...
public class RPSGame extends Application {
   ...
   public void run() {
      String menu = "To make a move enter: (R)ock, (P)aper or (S)cissors\nTo quit the game click Cancel.\nWhat is your move?";
      String status = "";
      String text = "";
      String optionList = "RPS";
      char myMove;
      char compMove; 
      
      while(true)
      {
         text = String.format("%s%n%s", status, menu);
         myMove = inputChar(text);
         myMove = Character.toUpperCase(myMove);
         
         if(myMove == ' ') break;
         
         // if the user's move is valid i.e. R, P or S
         if(optionList.contains(String.valueOf(myMove)))
         {
            compMove = optionList.charAt(getRandom(3));
            
            // update status with result of the game: tie, win or loss
            if(myMove == compMove)
            {
               status = String.format("%s%s\tIt's a tie!", myMove, compMove);
            }
            else if((myMove == 'R' && compMove == 'S') || 
                    (myMove == 'S' && compMove == 'P') ||
                    (myMove == 'P' && compMove == 'R'))
            {
               status = String.format("%s%s\tYou won!", myMove, compMove);
            }
            else 
               status = String.format("%s%s\tYou lost!", myMove, compMove);
         } // end if
         else
         {
            status = "Your move is not valid.";
         }
      }// end while loop
      outputln("Thanks for playing Rock, Paper, Scissors");
   } // end run
