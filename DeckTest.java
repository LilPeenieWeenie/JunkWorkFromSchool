...
public class DeckTest
 extends Application {
   ...
   private void setup() {
      form = new EntryPane("How many players", "Cards per player");
   ...
   public void run() {
      int players = Integer.parseInt(form.getField(0));
      int cardsPer = Integer.parseInt(form.getField(1));
   
      Deck deck = new Deck();
      
      for(int i = 0; i < players; i++)
      {
         Card[] hand = deck.dealHand(cardsPer);
         int handValue = 0;
         for(Card card: hand)
         {   
            outputln(card);
