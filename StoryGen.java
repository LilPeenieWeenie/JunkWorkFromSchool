...
public class StoryGen extends Application {
   ...
   String[] settings = {"a park", "a hospital", "a lab", "a bakery", "a suberb"};
   String[] genders = {"male", "female", "robot", "bi-gender", "alien"};
   String[] persons = {"writer", "pilot", "doctor", "teacher", "programmer"};
   String[] objects = {"a secret society", "a rollercoaster", "a tsunami", "a crystal ball", "a skyscraper"};
   String[] actions = {"tries to stop", "falls in love with", "swims beside", "laughs at", "gives a hug to"};

   public void run() {
      String setting = settings[getRandom(settings.length)];
      String gender = genders[getRandom(genders.length)];
      String person = persons[getRandom(persons.length)];
      String object = objects[getRandom(objects.length)];
      String action = actions[getRandom(actions.length)];
     
     String subject = gender + " " + person;
     
     outputln(String.format("In %s there is a %s who %s %s", setting, subject, action, object));
