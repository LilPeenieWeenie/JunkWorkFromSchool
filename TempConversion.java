...
public class TempConversion extends Application {
   ...
   private void setup() {
      form = new EntryPane("Enter temperature.", "Farenheight or Celcius (F / C)");
   ...
   public void run() {
      double temp = Double.parseDouble(form.getField(0));
      String tempTypes = form.getField(1);
      double answer = 0;
      tempTypes = tempTypes.toUpperCase();
      
      if(tempTypes.equals("C"))
      {
         answer = 9.0 * temp / 5.0 + 32;
         outputln(String.format("%f°C converts to %f°F", temp, answer)); 
      }
      else if(tempTypes.equals("F"))
      {
         answer = ((temp - 32) * 5) / 9.0;
         outputln(String.format("%.2f°F converts to %.2f°C", temp, answer));
      }
