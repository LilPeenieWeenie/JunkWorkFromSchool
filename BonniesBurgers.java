...
public class BonniesBurgers extends Application {
   ...
   public void run() {
      ...
      String menu = "Welcome to Bonnie's Burgers";
      ...
      choice = Character.toUpperCase(inputChar(menu));
      if(choice == ' ')
         choice = 'S';
      ...
      while(choice != 'S')
      {
         ...
         {
            case 'C':
               outputln(String.format("%-15s %6.2f", "Cheeseburger", 7.49));
               orderCost += 7.49;
               break;
            case 'H':
               outputln(String.format("%-15s %6.2f", "Hotdog", 6.99));
               orderCost += 6.99;
               break;
            case 'L':
               outputln(String.format("%-15s %6.2f", "Lemonade", 2.50));
               orderCost += 2.50;
               break;
            case 'T':
               outputln(String.format("%-15s %6.2f", "Iced Tea", 2.75));
               orderCost += 2.75;
               break;    
            default:
               outputln("Not a menu item -_-");
            
         }
         choice = Character.toUpperCase(inputChar(menu));
      } // end while loop
      if(orderCost > 0)
      outputln(String.format("%-15s %6.2f", "Total:", orderCost));
      else
      outputln("Why didn't you buy anything? ^-^");
   } // end run
