...
public class ChangeApp extends Application {
   ...
   private void setup() {
      form = new EntryPane("Pennies", "Nickels", "Dimes", "Quarters");
   ...
   public void run() {
      double pennies = Double.parseDouble(form.getField(0));
      double nickels = Double.parseDouble(form.getField(1));
      double dimes = Double.parseDouble(form.getField(2));
      double quarters = Double.parseDouble(form.getField(3));
      
      double total;
      
      total = pennies * .01 + nickels * .05 + dimes * .10 + quarters * .25;
      
      outputln(String.format("Your balance is $%f", total));
