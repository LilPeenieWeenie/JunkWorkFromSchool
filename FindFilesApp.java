...
public class FindFilesApp extends Application {
   ...
   private void setup() {
      form = new EntryPane("Enter directory", "Enter file type(.txt, .pdf, etc.)");
   ...
   public void run() {
      String dir = form.getField(0);
      String type = form.getField(1);
      
      dirSearch(new File(dir), type);
   
   } // end run

   // helper methods can go here
   /*
   Recursive fuctionto descend into the directory tree and
   find all the files that end with a given extention
   */
   public void dirSearch(File dir, String extension) {
      File[] fileList = dir.listFiles();
      if(fileList != null) {
         for(File file: fileList) {
            if(file.isDirectory())
               dirSearch(file, extension);
            else {
         ...
         outputln("Invalid directory erro");
